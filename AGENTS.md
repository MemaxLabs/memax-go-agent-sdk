...
- Do not copy code or implementation text from the references. Learn the design
  maturity, invariants, and tradeoffs, then implement the Memax-native version.
- Explicitly classify the target behavior as one of:
  - **Foundation**: correct extension point exists, but behavior is basic.
  - **Competitive**: matches the maturity expected from leading agents.
  - **Leading**: adds a clear advantage while preserving safety and DX.
- If our implementation intentionally differs from the references, document the
  reason in code comments, docs, or the final engineering summary. Differences
  should be due to product philosophy, Go SDK ergonomics, provider neutrality,
  filesystem neutrality, or host ownership, not because we forgot to check.
- Prefer progressive disclosure, explicit tool-mediated capability, durable
  transcript semantics, context provenance, and eval-backed behavior over
  prompt stuffing or hidden side effects.
- Treat every "intelligence" claim as incomplete until it has deterministic
  eval coverage. Good architecture without behavioral evals is not enough.

Use `docs/agent-runtime-quality.md` as the living gap analysis and execution
plan. Keep it current when a subsystem graduates from Foundation to Competitive
or Leading.
...
- Is the intended maturity explicit for this subsystem? Foundation work is
  acceptable when it is deliberately scoped, documented, and connected to the
  next step toward Competitive or Leading behavior.
