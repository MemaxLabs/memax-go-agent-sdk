...
- Add planner policies. Initial `planner.Policy`, `planner.Plan`, `planner.TaskSource`, `Options.Planner`, named `memax.plan` prompt injection, task-state adapter support through `toolkit/tasktools`, and planner-guided eval coverage exist for host-owned strategy and progress context.
- Add project/user memory injection. Initial `memory.Source`, `Options.MemorySource`, explicit `Options.Memories`, relevance selection, and named prompt-part injection exist for persistent project rules, user preferences, session notes, and organization context. Initial `memory.Writer`, `memory.Deleter`, in-memory memory store, and `toolkit/memorytools` search/save/delete tools exist for opt-in agent memory mutation. Initial `memory.Distiller`, `Options.MemoryDistiller`, and `EventMemoryCandidates` support exists for post-result memory candidate proposals without automatic writes.
