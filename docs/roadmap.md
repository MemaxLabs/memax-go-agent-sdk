...
- Add planner policies. Initial `planner.Policy`, `planner.Plan`, `planner.TaskSource`, verification hints, `Options.Planner`, named `memax.plan` prompt injection, task-state adapter support through `toolkit/tasktools`, and planner-guided eval coverage exist for host-owned strategy, verification, and progress context.
