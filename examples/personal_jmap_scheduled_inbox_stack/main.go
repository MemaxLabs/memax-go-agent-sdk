...
func runExample(ctx context.Context, w io.Writer) error {
	...
	if len(results) != 1 || !results[0].Created {
		return fmt.Errorf("scheduled trigger fire = %#v, want one created JMAP inbox triage run", results)
	}
	...
	if len(duplicateResults) != 1 {
		return fmt.Errorf("duplicate trigger fire = %#v, want existing JMAP inbox triage run", duplicateResults)
	}
