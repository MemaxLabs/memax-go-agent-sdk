...
func TestRunExampleShowsScheduledInboxTriageWorkflow(t *testing.T) {
	t.Parallel()
	...
	for _, want := range []string{
		...
		"result: Scheduled inbox triage sent the urgent Acme reply and recorded the occurrence so the same hourly trigger does not run twice.",
