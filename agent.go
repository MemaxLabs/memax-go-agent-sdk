		...
		for _, use := range uses {
			...
			out = append(out, model.Message{
				...
				ToolResult: &model.ToolResult{
					ToolUseID: use.ID,
					Name:      use.Name,
					Content:   "tool call was interrupted before a result was recorded; continue without relying on this tool output",
					IsError:   true,
				},
			})
		}
...
func emitCommandToolEvent(ctx context.Context, emit func(Event) bool, opts Options, sessionID string, turn int, result model.ToolResult) bool {
	...
	commandEvent := &CommandEvent{
		...
		SignalsProcessTree: metadatavalues.Bool(result.Metadata, model.MetadataCommandSignalsProcessTree),
...
func (n *assistantTextStreamNormalizer) prefix(leading, core string) string {
	...
	if newlines == 0 {
		if assistantStreamStartsPunctuation(first) {
			return ""
		}
		return whitespace
	}
	if newlines == 1 {
		if assistantStreamStartsPunctuation(first) && first != '/' {
			return ""
		}
		return "\n"
	}
	...
	if assistantStreamLinePreservesFollowingBreak(n.lastLineTrimmed) {
		return "\n\n"
	}
	if assistantStreamHasHorizontalWhitespace(whitespace) {
		return " "
	}
	if first != '/' && assistantStreamStartsPunctuation(first) {
		return ""
	}
...
func assistantStreamLinePreservesFollowingBreak(text string) bool {
	text = strings.TrimSpace(text)
	return assistantStreamLineStartsList(text) ||
		strings.HasPrefix(text, "#") ||
		strings.HasPrefix(text, ">") ||
		strings.HasPrefix(text, "|") ||
		strings.HasPrefix(text, "```")
