		...
		Base: memaxagent.Options{
			...
			Identity: identity.Identity{
				...
				Constraints: []string{
					...
					"search schedule metadata before loading full event details or changing calendar state",
					"keep durable memory concise, specific, and clearly reusable",
					"treat personal data, long-lived notes, outbound messages, and calendar mutations as approval-sensitive state",
				},
			},
			AppendSystemPrompt: "Recall durable user and project context before writing new memory. Search note, document, message-thread, and schedule metadata before loading full content or changing calendar state. Keep task state explicit, prefer concise summaries, and use approvals before mutating long-lived personal context.",
		},
		...
		Base: memaxagent.Options{
			...
			Identity: identity.Identity{
				...
				Constraints: []string{
					...
					"search schedule metadata before reading event details or changing calendar state",
			...
			AppendSystemPrompt: "Use scoped delegation for independent research threads when it helps. Search note, document, message-thread, and schedule metadata before loading larger items or changing calendar state. Separate working notes from durable memory, keep conclusions traceable, and avoid saving long-lived memories without clear future value.",
		},
		SkillDisclosure: skill.DisclosureProgressive,
		Approval: approvaltools.Config{
			ConcurrencySafe: true,
